...
const AddTourTrip = () => {
  const { addTourTrip, uploadImage, regions } = useAppData();
  ...
  const [formData, setFormData] = useState({
    ...
    itinerary: [], 
    pricingInfo: { currency: 'USD', originalPrice: '', sellingPrice: '', discountPercentage: '', pricePer: 'Person', notes: '' },
  ...
  const [expandedSection, setExpandedSection] = useState('core'); // Manage which top-level section is open to save space
  ...
  // Nested Array Builders (Equipment Items)
  const addEquipItem = (catIndex) => setFormData(prev => {
    const newEq = [...prev.equipment];
    newEq[catIndex].items.push({ name: '', description: '', required: true, quantity: 1 });
    return { ...prev, equipment: newEq };
  });
  const updateEquipItem = (catIndex, itemIndex, field, value) => setFormData(prev => {
    const newEq = [...prev.equipment];
    newEq[catIndex].items[itemIndex][field] = value;
    return { ...prev, equipment: newEq };
  });
  const removeEquipItem = (catIndex, itemIndex) => setFormData(prev => {
    const newEq = [...prev.equipment];
    newEq[catIndex].items = newEq[catIndex].items.filter((_, i) => i !== itemIndex);
    return { ...prev, equipment: newEq };
  });
  ...
  const SectionHeader = ({ title, icon: Icon, sectionId, count = null }) => (
    <div className="flex justify-between items-center p-6 cursor-pointer hover:bg-gray-50 transition-colors select-none" onClick={() => setExpandedSection(expandedSection === sectionId ? null : sectionId)}>
      ...
      {expandedSection === sectionId ? <ChevronDown size={24} className="text-gray-400"/> : <ChevronRight size={24} className="text-gray-400"/>}
    </div>
  );
    ...
    <div className="max-w-6xl mx-auto pb-20">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          <SectionHeader title="Core Information & Facts" icon={Info} sectionId="core" />
          {expandedSection === 'core' && (
            <div className="p-6 border-t border-gray-100 space-y-8 bg-gray-50/30">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          <SectionHeader title="Pricing & Departures" icon={DollarSign} sectionId="pricing" count={formData.groupPricing.length + formData.departures.length} />
        ...
        {/* SECTION: Itinerary */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          <SectionHeader title="Itinerary Builder" icon={MapPin} sectionId="itinerary" count={formData.itinerary.length} />
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          <SectionHeader title="Inclusions & Exclusions" icon={CheckCircle2} sectionId="inc_exc" count={formData.inclusions.length + formData.exclusions.length} />
          {expandedSection === 'inc_exc' && (
            <div className="p-6 border-t border-gray-100 bg-gray-50/30 grid grid-cols-1 md:grid-cols-2 gap-8">
              {/* Inclusions */}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          <SectionHeader title="Route Map & Media" icon={ImageIcon} sectionId="route_map" />
          {expandedSection === 'route_map' && (
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          <SectionHeader title="Add-Ons & Essential Info" icon={Puzzle} sectionId="addons" count={formData.addOns.length + formData.essentialInfo.length} />
          {expandedSection === 'addons' && (
            <div className="p-6 border-t border-gray-100 bg-gray-50/30 space-y-8">
              {/* Add-Ons */}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          <SectionHeader title="Equipment List & FAQs" icon={Briefcase} sectionId="equip_faq" count={formData.equipment.length + formData.faqs.length} />
          {expandedSection === 'equip_faq' && (
            <div className="p-6 border-t border-gray-100 bg-gray-50/30 space-y-8">
              {/* Equipment */}
              <div className="border border-gray-200 bg-white rounded-xl p-5 shadow-sm">
        ...
        {/* Media & Route Map are now integrated in the sections above */}
        {/* Publish */}
