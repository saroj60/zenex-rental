...
import { Users, Gauge, Fuel, MessageCircle, Star, ArrowRight } from 'lucide-react';
import { Link } from 'react-router-dom';
import { useCurrency } from '../context/CurrencyContext';

const HiacePricingCard = () => {
  const { formatPrice } = useCurrency();

  const handleBook = () => {
    const message = `Hi Zenex Travel, I'm interested in booking a Toyota Hiace. Could you please provide more details on availability?`;
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm border border-sky-tint hover:shadow-md transition-shadow group flex flex-col w-full max-w-sm mx-auto mb-12">
      <Link to="/vehicles/hiace-routes" className="relative overflow-hidden rounded-lg mb-4 block cursor-pointer">
        <img 
          alt="Toyota Hiace" 
      ...
      <h3 className="font-headline-md text-2xl font-bold text-gray-900 mb-1">Toyota Hiace</h3>
      <div className="flex items-center gap-1.5 mb-4">
        <span className="text-gray-500 text-sm font-medium">Starting from</span>
        <span className="text-xl font-bold text-[#e53a24]">{formatPrice(8000)}</span>
      ...
      <div className="grid grid-cols-2 gap-2 mb-6 mt-auto">
        <div className="bg-[#f0f4f8] rounded-xl p-3 text-center flex flex-col items-center justify-center">
          ...
          <span className="text-sm font-semibold text-[#1e3a8a]">14 Seats</span>
        </div>
        <div className="bg-[#f0f4f8] rounded-xl p-3 text-center flex flex-col items-center justify-center">
        ...
        <div className="bg-[#f0f4f8] rounded-xl p-3 text-center flex flex-col items-center justify-center relative overflow-hidden">
          ...
          <span className="text-sm font-semibold text-[#1e3a8a]">4 Bags</span>
        </div>
      </div>

      <div className="flex flex-col gap-3 mt-auto">
        <Link 
          to={`/vehicles/hiace-routes`} 
...
export default HiacePricingCard;
