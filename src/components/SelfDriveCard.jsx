...
const SelfDriveCard = ({ isSmall = false }) => {
  ...
  return (
    <div className={`bg-white shadow-sm border border-sky-tint hover:shadow-md transition-shadow group flex flex-col w-full ${
      ...
      <Link to="/vehicles/self-drive" className={`relative overflow-hidden rounded-lg block cursor-pointer ${isSmall ? 'mb-2.5' : 'mb-4'}`}>
        <img 
          alt="Self Drive Cars" 
      ...
      <h3 className={`font-headline-md font-bold text-gray-900 mb-1 ${isSmall ? 'text-lg' : 'text-2xl'}`}>Self Drive Cars</h3>
      <div className={`flex items-center gap-1.5 ${isSmall ? 'mb-3' : 'mb-4'}`}>
        ...
        <span className={`font-bold text-[#e53a24] ${isSmall ? 'text-lg' : 'text-xl'}`}>{formatPrice(5000)}</span>
